...
# from video_upload import upload_file_to_youtube
...
def get_article(use_article=None):
    ...
    while True:
        ...
        if use_article is None and len(wiki_page_content) < MINIMUM_ARTICLE_LENGTH:
            ...
            if USE_PROMPTS:
                print(f"\naccept article {wiki_page_title}? y/n:")
                res = input()
                if res.lower() == 'n':
                    continue
            break
...
def summarize_article(wiki_page_content):
    ...
    try:
        ...
        print("Problem with results from smmry API!")
        print("Exiting")
        exit(1)
...
def get_random_clips(keywords, wiki_page_title):
    ...
    while number_got < number_of_random_videos_to_get:
        ...
        try:
            ...
            print(str(e))
            print("Can't get video of correct resolution")
            continue
        ...
        if video is not None:
            ...
            try:
                ...
                print("Error downloading video clips")
                print(str(e))
                exit(1)
            ...
            for i in range(0, 5):
                ...
                if USE_MOVIEPY_VIDEO_FX:
                    ...
                    video_fx_funcs = [
                        ...
                        # lambda clip: clip.fx(vfx.blink, param1, param2),
...
def detect_and_sort_faces(images_list):
    print(f"Detecting faces ({len(images_list)})", end="")
    ...
    for i, img in enumerate(images_list):
        ...
        for (x, y, w, h) in faces:
            ...
            # create a random float between 0.4 and 0.6
            rand_float = random.uniform(0.4, 0.6)
            cv2.circle(
                mask,
                (int(x + w / 2), int(y + h / 2)),
        ...
        # interval_func_choices = ["random", "edges", "threshold", "waves"]
        # sorting_func_choices = ["lightness", "hue", "saturation", "intensity", "minimum"]
        # rand_angle = random.randint(360)
        # rand_sort_amt = random.randint(75)
...
def make_narration(text):
    ...
    # print(f"# of voices available: {len(voices)}")
...
def make_video(use_article=None):
    ...
    print(f"Using soundtrack: {chosen_soundtrack}")
    ...
    print("Cleaning up images and audio...")
    for f in frames:
        try:
            f.close()
        except:
    ...
    print(f"Done, generated {movie_title}.mp4")
...
def cleanup():
    try:
        ...
        print("Cannot remove downloads folder")
        print(str(e))
    ...
    try:
        ...
        print("Cannot remove videos folder")
        print(str(e))

    try:
        os.remove("narration.mp3")
    except Exception as e:
        print("Cannot remove narration.mp3")
        print(str(e))
...
def open_ai_stuff(topic):
    ...
    while not accepted_opening:
        ...
        if USE_PROMPTS:
            print(f"Proposed opening: {opening}\nAccept? y/n:", end='')
            res = input('')
            if res.lower() == 'y':
    ...
    while not accepted_closing:
        ...
        if USE_PROMPTS:
            print(f"Proposed closing: {closing}\nAccept? y/n:", end='')
            res = input('')
            if res.lower() == 'y':
